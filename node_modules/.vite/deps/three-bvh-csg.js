...
function computeDisjointEdges(geometry, unmatchedSet, eps) {
  ...
  for (let i = 0, l = edges.length; i < l; i++) {
    ...
    toNormalizedRay(_v0, _v1, _ray4);
    let info;
    let commonRay = rays.findClosestRay(_ray4);
    if (commonRay === null) {
      commonRay = _ray4.clone();
    ...
    if (_ray4.direction.dot(commonRay.direction) < 0) {
...
function getHitSide(tri, bvh) {
  tri.getMidpoint(_ray5.origin);
  tri.getNormal(_ray5.direction);
  const hit = bvh.raycastFirst(_ray5, DoubleSide);
  const hitBackSide = Boolean(hit && _ray5.direction.dot(hit.face.normal) > 0);
...
function getHitSideWithCoplanarCheck(tri, bvh) {
  ...
  _ray5.direction.copy(_normal);
  tri.getMidpoint(_ray5.origin);
  const total = 3;
  let count = 0;
  let minDistance = Infinity;
  for (let i = 0; i < total; i++) {
    _ray5.direction.x += rand() * JITTER_EPSILON;
    _ray5.direction.y += rand() * JITTER_EPSILON;
    _ray5.direction.z += rand() * JITTER_EPSILON;
    _ray5.direction.multiplyScalar(-1);
    const hit = bvh.raycastFirst(_ray5, DoubleSide);
    let hitBackSide = Boolean(hit && _ray5.direction.dot(hit.face.normal) > 0);
    if (hitBackSide) {
      count++;
    }
    if (hit !== null) {
...
function getOverlapEdge(tri1, e1, tri2, e2, target) {
  ...
  toNormalizedRay(v0_1, v1_1, _ray6);
  let d0_1 = _vec5.subVectors(v0_1, _ray6.origin).dot(_ray6.direction);
  let d1_1 = _vec5.subVectors(v1_1, _ray6.origin).dot(_ray6.direction);
  if (d0_1 > d1_1) [d0_1, d1_1] = [d1_1, d0_1];
  let d0_2 = _vec5.subVectors(v0_2, _ray6.origin).dot(_ray6.direction);
  let d1_2 = _vec5.subVectors(v1_2, _ray6.origin).dot(_ray6.direction);
  if (d0_2 > d1_2) [d0_2, d1_2] = [d1_2, d0_2];
  const final_0 = Math.max(d0_1, d0_2);
  const final_1 = Math.min(d1_1, d1_2);
  _ray6.at(final_0, target.start);
  _ray6.at(final_1, target.end);
}
